...
public class Automata {
  ...
  private int valueStoreVersion;
  private int persistenceVersion;
  private int persistenceSize;
  private IndexInput in;

  public Automata(InputStream inputStream) throws Exception {
    for (int i = 0; i < KeyviConstants.KEYVI_FILE_MAGIC.length; ++i) {
      if (inputStream.read() != KeyviConstants.KEYVI_FILE_MAGIC[i]) {
        // todo: custom exception
        throw new Exception("invalid keyvi file");
      }
    }    
  }
